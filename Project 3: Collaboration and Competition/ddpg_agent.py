#################################################################################################
# https://github.com/udacity/deep-reinforcement-learning/tree/master/ddpg-bipedal/ddpg_agent.py #
#################################################################################################
...
UPDATE_EVERY = 3        # how often to update the networks in time steps
...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done):
        ...
        for n in range(self.num_agents):
            self.memory.add(state[n, :], action[n, :], reward[n], next_state[n, :], done[n])
    ...
    def act(self, states, add_noise=True):
        ...
        with torch.no_grad():
            for agent, state in enumerate(states):
                actions[agent, :] = self.actor_local(state).cpu().data.numpy()
